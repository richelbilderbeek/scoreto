test_that("use", {
  ...
  from_dates <- extract_english_from_dates(english_date_ranges)
  expect_equal(length(english_date_ranges), length(from_dates))
  expect_false(all(stringr::str_detect(from_dates, "NA")))
})
